...
# --- STEP 2: PROCESS STEERING & CURVATURE DATA (SINGLE PASS PER OUTING) ---
laps_steer_o1, steer_speed_o1, laps_curv_o1, curvature_o1 = (
    process_outing_steering_and_curvature(
        outing1_files, min_steering_rate=MIN_STEER_THRESHOLD, min_lat_acc_g=0.2, min_speed_kmh=30.0
    )
)
laps_steer_o2, steer_speed_o2, laps_curv_o2, curvature_o2 = (
    process_outing_steering_and_curvature(
        outing2_files, min_steering_rate=MIN_STEER_THRESHOLD, min_lat_acc_g=0.2, min_speed_kmh=30.0
    )
)
...
if laps_steer_o1:
    ax1.scatter(laps_steer_o1, steer_speed_o1, color=COLORS["Outing 1"], label="Outing 1", **SCATTER_STYLE)
if laps_steer_o2:
    ax1.scatter(laps_steer_o2, steer_speed_o2, color=COLORS["Outing 2"], label="Outing 2", **SCATTER_STYLE)

ax1.set_title(f"Average Absolute Steering Speed per Lap (≥{MIN_STEER_THRESHOLD:.0f} deg/s)", fontsize=12, fontweight="bold", pad=12)
...
if laps_curv_o1:
    ax2.scatter(laps_curv_o1, curvature_o1, color=COLORS["Outing 1"], label="Outing 1", **SCATTER_STYLE)
if laps_curv_o2:
    ax2.scatter(laps_curv_o2, curvature_o2, color=COLORS["Outing 2"], label="Outing 2", **SCATTER_STYLE)

ax2.set_title("Average Trajectory Curvature per Lap (r = |G_lat| / V²)", fontsize=12, fontweight="bold", pad=12)
...
# Format discrete integer X-ticks across both subplots
all_laps = sorted(list(set((laps_steer_o1 or ()) + (laps_steer_o2 or ()) + (laps_curv_o1 or ()) + (laps_curv_o2 or ()))))
if all_laps:
