...
# --- STEP 2: PROCESS BRAKE DYNAMICS (SINGLE PASS PER OUTING) ---
...
if laps_app_o1:
    ax1.scatter(laps_app_o1, brake_app_speed_o1, color=COLORS["Outing 1"], label="Outing 1", **SCATTER_STYLE)
if laps_app_o2:
    ax1.scatter(laps_app_o2, brake_app_speed_o2, color=COLORS["Outing 2"], label="Outing 2", **SCATTER_STYLE)

ax1.set_title("Average Brake Application Speed per Lap (>80%/s)", fontsize=12, fontweight="bold", pad=12)
ax1.set_xlabel("Lap Number", fontsize=11, fontweight="bold")
ax1.set_ylabel("Application Speed (%/s)", fontsize=11, fontweight="bold")
ax1.set_ylim(bottom=80, top=220)
ax1.grid(True, linestyle="--", alpha=0.6)
ax1.legend(loc="best", fontsize=10)

# Subplot 2: Release Speed (Trail-braking smoothness)
if laps_rel_o1:
    ax2.scatter(laps_rel_o1, brake_rel_speed_o1, color=COLORS["Outing 1"], label="Outing 1", **SCATTER_STYLE)
if laps_rel_o2:
    ax2.scatter(laps_rel_o2, brake_rel_speed_o2, color=COLORS["Outing 2"], label="Outing 2", **SCATTER_STYLE)

ax2.set_title("Average Brake Release Speed per Lap (<10%/s)", fontsize=12, fontweight="bold", pad=12)
...
all_laps = sorted(list(set((laps_app_o1 or ()) + (laps_app_o2 or ()) + (laps_rel_o1 or ()) + (laps_rel_o2 or ()))))
if all_laps:
